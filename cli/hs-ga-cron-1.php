...
$limit = 30;
$logFileName = dirname( dirname(__FILE__) ) . '/log/hs-ga-cron-1_' . date('Y-m-d') . '.log';
...
try {
    ...
    $json1 = json_decode($recCreDeals, true);
    // $msg = date('r') . ' ' . __FILE__ . ' +' . __LINE__ . ' Recently created deals: ' . var_export($json1, 1) . PHP_EOL;
    // logMsg( $msg, $logFileName, ['echoLogMsg' => false, 'storeLog' => true] );

    if (!empty($json1['results'])) {
        foreach ($json1['results'] as $r0) {
            $msg = date('r') . ' ' . __FILE__ . ' +' . __LINE__ . ' Deal data: ' . var_export($r0, 1) . PHP_EOL;
            logMsg( $msg, $logFileName, ['echoLogMsg' => true, 'storeLog' => true] );
            //
