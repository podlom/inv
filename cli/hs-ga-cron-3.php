...
$limit = 30;
$logFileName = dirname( dirname(__FILE__) ) . '/log/hs-ga-cron-3.log';
...
if ( isset($argv[1])
    ...
    $keyId = intval($argv[1]);
    $logFileName = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . 'hs-ga-cron-3_' . $keyId . '.log';
}
...
try {
    ...
    if (!is_null($keyId)) {
        ...
        $query = "SELECT * FROM `" . DB_TABLE . "` WHERE `status` = 'lost_deal' AND `key_id` = '{$keyId}' LIMIT {$limit} OFFSET 0 FOR UPDATE";
    } else {
        $query = "SELECT * FROM `" . DB_TABLE . "` WHERE `status` = 'lost_deal' LIMIT {$limit} OFFSET 0 FOR UPDATE";
    }
    $res = $db->query($query);
    //
    ...
    if (!empty($res)) {
        foreach ($res as $r1) {
            ...
            //
            $dealLostReason = '';
            $dealData2 = $hubSpot->getDealById($r1['deal_id']);
            $json2 = json_decode($dealData2, 1);
            if (isset($json2['properties']['closed_lost_reason']['value']) && !empty($json2['properties']['closed_lost_reason']['value'])) {
                $dealLostReason = $json2['properties']['closed_lost_reason']['value'];
                //
                $msg = date('r') . ' ' . __FILE__ . ' +' . __LINE__ . ' Deal lost reason: ' . var_export($dealLostReason, 1) . PHP_EOL;
                logMsg( $msg, $logFileName, ['echoLogMsg' => true, 'storeLog' => true] );
            }
            //
            ...
            $ea = urlencode(trim($pipelineStatusName . ' [Lost]'));
            if (!empty($r1['ea'])) {
                $ea = $r1['ea'];
            }
            ...
            $cd3 = urlencode(trim($dealLostReason));
            //
            $hitUrl .= '&tid=' . $tid;
            $hitUrl .= '&cid=' . $cid;
            $hitUrl .= '&t=' . $type;
