...
$limit = 30;
$logFileName = dirname( dirname(__FILE__) ) . '/log/hs-ga-cron-5.log';
...
if ( isset($argv[1])
    ...
    $keyId = intval($argv[1]);
    $logFileName = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . 'hs-ga-cron-5_' . $keyId . '.log';
}
...
try {
    ...
    if (!is_null($keyId)) {
        ...
        $query = "SELECT * FROM `" . DB_TABLE . "` WHERE `status` = 'updated_deal' AND `key_id` = '{$keyId}' LIMIT {$limit} OFFSET 0 FOR UPDATE";
    } else {
        $query = "SELECT * FROM `" . DB_TABLE . "` WHERE `status` = 'updated_deal' LIMIT {$limit} OFFSET 0 FOR UPDATE";
    }
    $res = $db->query($query);
    //
    ...
    if (!empty($res)) {
        foreach ($res as $r1) {
            ...
            $ea = urlencode(trim($pipelineStatusName . ' [Update]'));
            if (!empty($r1['ea'])) {
                $ea = $r1['ea'];
            }
