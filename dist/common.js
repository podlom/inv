...
$(document.body).ready(function() {
	if ($('.section__category').length) {
		...
		$('.section__category').each(function(el) {
			...
			if (sortType) {
				...
				$(this).attr(
					'href',
					''.concat(_link, '&filter[sort]=').concat(sortType),
				);
			}
		});
	}
	...
	if ($('.preview__name-category').length) {
		...
		if (
			Array.isArray(_link2) &&
			_link2.length &&
			(_link2[0] === 'investments' || _link2[1] === 'investments')
		) {
			$('.preview__name-category a').attr(
				'href',
				'/investments?filter[category][parent]='.concat(_link2[1]),
			);
		}
	}
...
$(document).ready(function() {
  ...
  $('select[name="price1"] option').each((counter, option) => {
	...
	var groupParamsByKey = function groupParamsByKey(params) {
		return _toConsumableArray(params.entries()).reduce(function(
			acc,
			tuple,
		) {
	...
	if ($('#search-page #sort-select').length) {
		...
		$('#sort-select li[data-value="'.concat(sortType, '"]')).addClass(
			activeClass,
		);
	}
	...
	if ($('.post__video').length) {
		...
		if (elementToHide) {
			$.get(
				'/page-attr-data.php?page_id='.concat(
					postId,
					'&action=get_text',
				),
			).then(function(data) {
