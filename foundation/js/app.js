			...
			.then(json => {
				...
				switch(action){
					case "/user/login":
						...
						if(json.error)
							$(frm).find('[type=submit]').prev('.server-error').addClass('show').text(json.error)
					...
					case "/user/register":
						...
						if(json.user){
							var userEmail = $('#singUp').find('input[type=email]').val()
							$('#confirmMailCode').foundation('open')
							$('#confirmMailCode').find('[name=email]').val(userEmail)
						}
						if(json.error)
							$(frm).find('[type=submit]').prev('.server-error').addClass('show').text(json.error)
						...
						break;
					case "/user/restore":
						frm.parent().siblings('.modal-subtitle')
							.css('marginBottom', 0)
							.find('.or')
							.html('Ссылка восстановления пароля отправлена на email <b>' +  json.email + '</b>')
						frm.parent().remove()
					...
					case "/user/confirm":
						if(json.result){
							...
							$(frm).find('.form-error').text(json.error).addClass('is-visible')
							$(frm).find('label').addClass('is-invalid-label')
							$(frm).find('[name=code]').addClass('is-invalid-input')
						}
						$(frm).find('[type=submit]').prop('disabled', false)
						break;
					case "/participate":
			...
			fetch('/vote/' + this.vote + '.json')
				...
				.then(json => {
					...
					if(json.can_vote_again){
						$(target.el).removeClass('hollow').html('<i class="fi fi-thumbs-up-solid"></i> Ваш голос учтен')
						setTimeout(()=>{
							$(target.el).html('<i class="fi fi-thumbs-up-solid"></i> Проголосовать еще')
						}, 3000)
					}
					else{
						$(target.el).removeClass('hollow').prop('disabled', true).html('<i class="fi fi-thumbs-up-solid"></i> Ваш голос учтен')
					}
...
window.cardsConfirm = {
	...
	init() {
		...
		$.each(this.cards, (index, item) => $('[data-bookmark='+item+']').addClass('added'))

		this.url = JSON.stringify(this.cards).replace('[', '').replace(']', '')
...
$('[data-bookmark]').click(function(e){
	...
	if(target.attr('delete')){
		...
		if(confirmChoice) {
			...
			var pageUrl = window.location.origin + '/mychoice/' + sessionStorage.getItem('voteCards').replace('[', '').replace(']', '')
			window.history.replaceState({}, 'Конкурс | СРавнение', pageUrl)
			target.closest('.card').parent('.cell').remove()
		}
  ...
  if (input.files && input.files.length > 0) {
    ...
    img.onload = function() {
        if(this.width < maxwidth || this.height < maxheight) {
        	alert('Размер изображения должен быть не меньше ' + maxwidth + 'x' + maxheight + 'px')
        }else {
	...
	fetch('/upload/' + imegeName, {
  	...
  	.then(json => {
	  		...
	  		.then(json => {
	  			...
	  			$('.upload-btn-wrapper > .dd-title').text('Фото успешно загружено').siblings().remove()
	  			setTimeout(()=>{
	  				location.reload()
	  			}, 1500) 
	  		})
...
function mekeRandomeName(imageName) {
	...
	first.toString(36)
	second.toString(36)
	const newName = first.toString(36).substr(2, 9) + second.toString(36).substr(2, 9) + '.' + extension
	return newName
}
...
$(function(){
	...
	$(window).scroll(function(){
		...
		if ($(this).scrollTop() > 5) { // 300px from top
