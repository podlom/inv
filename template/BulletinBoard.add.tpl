...
{block 'content'}
    ...
    <div class="viewbox-cnt-body board_edit">
        ...
        <div class="rules">
            <p>Цена размещения объявления – <b>99 грн</b></p>
            <p>Срок размещения объявления – <b>3 месяца</b></p>
            <p>Ознакомиться с <a href="/rules">правилами размещения объявлений</a></p>
        </div>
        {content $page}
        {$form->open()}
            ...
            <div class="add_img">
                <label class="label_one">{$form->getField('image')->getInput()->setAttr('id','imgInput')}<span>{if $post->getImage()}Изменить{else}Добавить{/if} изображение</span></label>
                <div></div>
