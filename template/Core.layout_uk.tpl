...
<!--[if gt IE 8]><!-->
<html lang="{$lang|default:'uk'}" prefix="og: https://ogp.me/ns#">
<!--<![endif]-->
...
<head>
	...
	<link rel="icon" href="/images/theme/favicon/touch-icon-iphone.png" sizes="60x60" type="image/png" />
	<link rel="icon" href="/images/theme/favicon/touch-icon-iphone-retina.png" sizes="120x120" type="image/png" />
	...
	{* 2. Визначаємо мовний префікс *}
	{assign var="langPrefix" value=""}
	{if $currentPath|regex_match:"^/uk"}
		{assign var="langPrefix" value="uk"}
	{elseif $currentPath|regex_match:"^/en"}
		{assign var="langPrefix" value="en"}
	{/if}

	{* 3. Видаляємо мовний префікс, щоб отримати basePath *}
	{assign var="basePath" value=$currentPath|regex_replace:"^/(uk|en)\/?":""}

	{* 4. Готуємо hreflang-посилання *}
    ...
    || $request->getPathInfo() =='/digest-social'
	...
	{if $request->getPathInfo() == '/uk/investments/prodazh-diyuchoyi-merezhi-myasnih-magaziniv-z-vlasnim-virobnictvom-logistikoyu-ta-franshizoyu'}
		{literal}
...
<body>
	...
	<div class="off-canvas-wrap" data-offcanvas="">
		<div
			class="inner-wrap {if $request->getPathInfo() == '/uk/add-inv-prop' || $request->getPathInfo() == '/uk/investor' }pt-0{/if}">
			{block content}
				{$content}
			{/block}
...
*}
	{if $user && $user->canAccess('Page.publish') && ($page || $post || $blog)}
		...
		jQuery('.admin_edit').append('<br><button class="save-inline-edits">Зберегти зміни</button>');
