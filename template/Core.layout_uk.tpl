...
<!--[if gt IE 8]><!--><html lang="{$lang|default:'uk'}" prefix="og: https://ogp.me/ns#" ><!--<![endif]-->
<head>
{if $sm}
{$user = $sm->getUser()}
{/if}
...
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-T3V7BCL"
...
{/literal}
  ...
  {/if}
    ...
    <div class="off-canvas-wrap" data-offcanvas="">
        <div class="inner-wrap {if $request->getPathInfo() == '/uk/add-inv-prop' || $request->getPathInfo() == '/uk/investor' }pt-0{/if}">
            {block content}
                {$content}
            {/block}
...
{literal}
    jQuery.post('/invest.php', {'action':'set','data':jQuery('.the_post_content').html(),'path':window.location.pathname}, function(res1){ console.log('+824 Got result: ' + res1); });
{/literal}
  });
