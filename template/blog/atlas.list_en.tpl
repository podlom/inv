...
<div class="filter m_b-15" >
    ...
    <form name="atlas-form" method="get" action="">
        <h3><i class="fa fa-search"></i>Investor search</h3>
        <div class="row">
            <div>
                {$filter->getField('search')->getInput()->setAttr('placeholder', "Search by name")}
            </div>
            <div>
                <label class="label-click1 label-input">Sector<span class="open icon"></span><span class="close icon">&#215;</span></label>
                <div class="tged1 hide otr-inv">
                    {$filter->getField(16)->getInput()}
                </div>
            </div>
            <div>
                <label class="otr-invtitle label-click2 label-input">Stage<span class="open icon"></span><span class="close icon">&#215;</span></label>
                <div class="otr-inv tged2 hide">
                    {$filter->getField(53)->getInput()}
                </div>
            </div>
            <div>
            <label class="otr-invtitle label-click3 label-input">Investor profile<span class="open icon"></span><span class="close icon">&#215;</span></label>
                <div class="otr-inv tged3 hide">
                    {$filter->getField(46)->getInput()}
                </div>
            </div>
           <div>
                <label class="label-click6 label-input">Investment amount<span class="open icon"></span><span class="close icon">&#215;</span></label>
                <div class="tged6 otr-inv">{$filter->getField(60)->getInput()->setAttr('placeholder', "Размер инвестиций")}</div>
            </div>
            <div class="columns">
                <center><button class="blue_but" type="submit" aria-label="submit form">Search</button></center>
            </div>
        </div>
    </form>
</div>
