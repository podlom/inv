    ...
    <div class="row m_b-20 auto-height">
        ...
        {block 'post.image'}
            ...
            <div id="thumbBig" class="reveal-modal large medium" data-reveal>
                <img itemprop="image" itemscope itemtype="http://schema.org/URL" itemtype="http://schema.org/ImageObject" src="{$post->getImage()->getUrl()}" alt='{$post->getTitle()}' />
    	...
    	<div style="position: relative;">
	        ...
	        <div class="event_time">
			    <ul>
			        ...
			        {if ($post->getAttr(10))}<li><b>Address</b>: <span  itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">{$post->printAttr(10)}</span></li>{/if}
			        {if ($post->getAttr(6))}<li><b>Website</b>: <a href="{$post->getAttr(6)}" itemprop="url" target="_blank">{$post->getAttr(6)}</a></li>{/if}
			    </ul>
			</div>
		</div>
          <div class="wrapper">
