<section class="grid-container inventure_list inventure_list_en">
  <div class="grid-x grid-margin-x">
    <div class="cell large-9 medium-12">
                ...
                <div class="row">
                    ...
                    {if !$category}
                            <select onchange="this.form.action=this.value">
                                <option value="/investments">Type</option>
                                {foreach $blog->getCategories() as $cat}
                                    <option value="{$cat->getPath()}">{$cat->getH1()}</option>
                                {/foreach}
                            </select>
                    {/if}
                            {$filter->getField(10)->getInput()->setAttr('placeholder','Location')}
                    {if !$category || in_array($category->getSubpath(), ['projects', 'business', 'offer'])}
                        <div>
                            <label class="otr-invtitle label-click4 field-int">Industry<span class="open icon">&#9660;</span><span class="close icon">&#215;</span></label>
                    ...
                    <div class="columns my_button">
                        <button class="blue_but" type="submit" aria-label="submit form">Search</button>
                    </div>
        ...
        {block 'menu'}
            <nav>
                <span {if $request->getPathInfo() == '/investments/projects'}class="active"{/if}><a href="/en/investments/projects">Investment Projects and Startups</a></span> |
                <span {if $request->getPathInfo() == '/investments/business'}class="active"{/if}><a href="/en/investments/business">Business for Sale</a></span> |
                <span {if $request->getPathInfo() == '/investments/realestate'}class="active"{/if}><a href="/en/investments/realestate">Commercial Property</a></span> |
                <span {if $request->getPathInfo() == '/investments/land'}class="active"{/if}><a href="/en/investments/land">Land</a></span> |
                <span {if $request->getPathInfo() == '/investments/offer'}class="active"{/if}><a href="/en/investments/offer">Investor Offers</a></span>
            </nav>
...
{$sorts = [
    [['filter'=>['sort'=>'published']], 'Latest'],
    [['filter'=>['sort'=>'views']], 'Popular'],
    [['filter'=>['sort'=>'rating']], 'Featured']
]}
        ...
        {/block}
            <div class="row">
                <div class="auto-height">
                    ...
                    {/if}
                        {block 'post'}
                    <div style="position: relative; height: 475px; margin-bottom: 24px;">
                        <div class="box-shadow"></div>
                    <div class="item ">
                        {block 'image'}
                        {if $post->getImage()}
                      ...
                      <div class="position">
                        ...
                        <div class="footer">
                            ...
                            {if $post->getParent()->getId() == 9779 || $post->getParent()->getId() == 9780 ||$post->getParent()->getId() == 9781}
                              ...
                              {if $post->getAttr("15")!=''} ${$post->printAttr("15")|number_format:0:'.':' '}{else}Negotiable{/if}</p>
