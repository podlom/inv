												    ...
												    {foreach $posts as $post}
														...
														{* {$posts->setCount(24)}
														    {foreach $posts as $post}
															    ...
															    {/block}
															        {/foreach} *}
																        {block 'post.title'}<h2 class="cards__title" itemprop="headline">{$post->getH1()}</h2>
																	{* {$posts->setCount(24)}
																	    {foreach $posts as $post}
																		        {if $post->getAttr(58) == 1}
																			            {$image_class = 'inv-sold'}
																...
																{* {$posts->setCount(24)}
																    {foreach $posts as $post}
																	    ...
																	    {/block}
																	        {/foreach} *}
																		        {if trim($post->getAttr("15"))} ${$post->printAttr("15")|number_format:0:'.':' '}
																			{* {$posts->setCount(24)}
																			    {foreach $posts as $post}
																				        {if $post->getAttr(58) == 1}
																					            {$image_class = 'inv-sold'}
