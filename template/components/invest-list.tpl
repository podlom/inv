   ...
   <div class="invest-projects__grid cards">
      <a href="{$bigpost->getPath()}" class="cards__item card cards__item--big">
         <div class="cards__img-wrapper mb-4">
            {if $bigpost->getImage()}
              <img data-src="{$bigpost->getImage()->thumbup(600,400)->getUrl()}"  alt="{$bigpost->getH1()}" class="lazyimg cards__img">
            {else}
         ...
         <p class="cards__description">{$bigpost->getShortText()|truncate:150:"  ..."}</p>
          ...
          {/if}
          <div class="cards__meta flex justify-between mt-auto opacity-75 text-sm big-itm parent-category-id-{$bigpost->getParent()->getId()} ">
            {if $bigpost->getParent()->getId() == 7861 || $bigpost->getParent()->getId() == 7862 || $bigpost->getParent()->getId() == 7863 || $bigpost->getParent()->getId() == 9779 || $bigpost->getParent()->getId() == 9780 || $bigpost->getParent()->getId() == 9781}
              <p class="price-tag font-semibold text-sm attr15-len-{$bigpost->hasAttr("15")|strlen}">{if $bigpost->hasAttr("15")} {if $bigpost->getAttr("15")|strlen}${$bigpost->printAttr("15")|number_format:0:'.':' '}{else}Договорная{/if}{/if}</p>
            {elseif $bigpost->getParent()->getId() == 7860 || $bigpost->getParent()->getId() == 9778 || $bigpost->getParent()->getId() == 19303 || $bigpost->getParent()->getId() == 19304}
              <p class="price-tag font-semibold text-sm attr34-len-{$bigpost->hasAttr("34")|strlen}">{if $bigpost->hasAttr("34")} {if $bigpost->getAttr("34")|strlen}${$bigpost->printAttr("34")|number_format:0:'.':' '}{else}Договорная{/if}{/if}</p>
            {elseif $bigpost->getParent()->getId() == 7864 || $bigpost->getParent()->getId() == 9782}
              <p class="price-tag font-semibold text-sm attr31-len-{$bigpost->hasAttr("31")|strlen}">{if $bigpost->hasAttr("31")} {if $bigpost->getAttr("31")|strlen}${$bigpost->printAttr("31")|number_format:0:'.':' '}{else}Договорная{/if}{/if}</p>
            {/if} 
        ...
        {foreach $posts as $post}
          ...
          <a href="{$post->getPath()}" class="cards__item cards__item--horizontal-sm card">
            <div class="cards__img-wrapper bd-radius--default">
                {if $post->getImage()}
                  <img data-src="{$post->getImage()->thumbup(350,254)->getUrl()}"  alt="{$post->getH1()}" class="lazyimg cards__img mt-3 desktop-mt-0 border-4">
                {else}
                  <img  class="cards__img mt-3 desktop-mt-0 border-4 lazyimg" data-src="/img/resize.375.225/images/noThumb.jpg" alt="{$post->getH1()}"/>
                {/if}
            </div>
            <div class="flex flex-col w-full">  
              <div class="opacity-75 text-sm w-full cards__meta pt-0 mt-0 mobile-mt-2">{$post->getPublished()->format('d.m.y')}</div>
              ...
              <div class="cards__meta flex justify-between mt-auto opacity-75 text-sm ts-post-parent-id-{$post->getParent()->getId()} ">
                {if $post->getParent()->getId() == 7861 || $post->getParent()->getId() == 7862 || $post->getParent()->getId() == 7863 || $post->getParent()->getId() == 9779 || $post->getParent()->getId() == 9780 || $post->getParent()->getId() == 9781}
                    <p class="price-tag font-semibold text-sm attr15-len-{$post->hasAttr("15")}">{if $post->hasAttr("15")|mb_strlen}{if $post->getAttr("15")|strlen}${$post->printAttr("15")|number_format:0:'.':' '}{else}Договорная{/if}{/if}</p>
                {elseif $post->getParent()->getId() == 7860 || $post->getParent()->getId() == 9778 || $post->getParent()->getId() == 19303 || $post->getParent()->getId() == 19304}
                    <p class="price-tag font-semibold text-sm attr34-len-{$post->hasAttr("34")}">{if $post->hasAttr("34")|mb_strlen}{if $post->getAttr("34")|strlen}${$post->printAttr("34")|number_format:0:'.':' '}{else}Договорная{/if}{/if}</p>
                {elseif $post->getParent()->getId() == 7864 || $post->getParent()->getId() == 9782}
                    <p class="price-tag font-semibold text-sm attr31-len-{$post->hasAttr("31")}">{if $post->hasAttr("31")|mb_strlen}{if $post->getAttr("31")|strlen}${$post->printAttr("31")|number_format:0:'.':' '}{else}Договорная{/if}{/if}</p>
                {/if} 
                <div class="flex items-center ml-auto">
                  {if $post->getAttr(13)}
                  <div class="flex items-center mr-4">
