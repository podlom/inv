  ...
  <div class="investor__header mobile-pt-4 mobile-pb-4">
    <div class="container">
      <div class="investor__header-text">
        ...
        {if $lang === 'ru'}
          Заполните пожалуйста форму, это поможет нам определить ваши предпочтения и инвестиционные критерии для подбора наиболее подходящих предложений. Мы гарантируем соблюдение полной конфиденциальности.
        {else}
          Please fill in the form, it will help us to define your preferences and investment criteria for selecting the most suitable offers. We guarantee your privacy.
        {/if}
        </div>
      </div>
    </div>
      ...
      <div class="form__body">
        <div class="form__section relative z-1">
          <div class="form__heading">
            ...
            {if $lang === 'ru'}Контактные данные{else}Your contacts{/if}
            </div>
          </div>
          <div class="form__fields form__fields--inputs">
            <div class="form__label-wrap floating-label-wrap">
              <input
                type="text"
                name="sf_investor[firstname]"
                class="floating-label-field floating-label-field--s2"
                placeholder="{if $lang === 'ru'}Ваше имя{else}First name{/if}"
                required
              />
              <label class="floating-label">
              {if $lang === 'ru'}Ваше имя{else}First name{/if}
              </label>
            </div>
            <div class="form__label-wrap floating-label-wrap">
              <input
                type="text"
                name="sf_investor[lastname]"
                class="floating-label-field floating-label-field--s2"
                placeholder="{if $lang === 'ru'}Фамилия{else}Last name{/if}"
                required
              />
              <label class="floating-label">
            ...
            <div class="form__label-wrap floating-label-wrap">
              <input
                type="text"
                name="sf_investor[company]"
                class="floating-label-field floating-label-field--s2"
                placeholder="{if $lang === 'ru'}Компания{else}Company{/if}"
                required
              />
              <label class="floating-label">
              {if $lang === 'ru'}Компания{else}Company{/if}
              </label>
            </div>
            <div class="form__label-wrap floating-label-wrap">
              <input
                type="text"
                name="sf_investor[job]"
                class="floating-label-field floating-label-field--s2"
                placeholder="{if $lang === 'ru'}Должность{else}Position{/if}"
                required
              />
              <label class="floating-label">
                {if $lang === 'ru'}Должность{else}Position{/if}
              </label>
            </div>
            <div class="form__label-wrap floating-label-wrap">
              <input
                type="email"
                name="sf_investor[email]"
                class="floating-label-field floating-label-field--s2"
                placeholder="E-mail"
                required
              />
            ...
            <div class="form__label-wrap floating-label-wrap">
              {* <input
                ...
                required
              /> *}
              <input data-label="{if $lang === "ru"}Телефон{else}Mobile phone{/if}" class="floating-label-field floating-label-field--s2" type="tel" id="phone" name="sf_investor[phone]" required>
        ...
        <div class="form__section">
          <div class="form__heading ">
            <input
              class="inp-cbx"
              id="manager"
              name="sf_investor[manager]"
              type="checkbox"
              checked="checked"
            />
            <label
              class="cbx cbx--white cbx--medium"
              for="manager"
              type="checkbox"
            >
              <span>
              ...
              <span>
                {if $lang === 'ru'}Заказать звонок от менеджера{else}Order a call from the manager.{/if}
              </span>
            </label>
          </div>
          <div class="form__fields form__fields--light form__fields--small">
          {if $lang === 'ru'}
            Выбирая эту функцию, Вы заказываете звонок от менеджера компании, который сможет предоставить дополнительную консультацию по интересующим Вас вопросам и ускорить процесс подбора подходящего предложения.
          {else}
            By choosing, you order a call from a company manager, who will be able to provide additional advice on issues that interest you and speed up the process of selecting the right offer.
          {/if}
    ...
    <div class="form__footer footer">
      <div class="container">
        <div class="footer__wrapper">
          ...
          {if $lang === 'ru'}Отправить анкету{else}Send a form{/if}
          </button>
