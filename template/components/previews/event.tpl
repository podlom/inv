...
<div class="preview__wrapper  {if $post->getImage()}plain-text__card--with-img{else}plain-text__card--without-img{/if}">
    ...
    <div class="preview__wrapper-main" style="position: relative;" itemprop="about" itemscope itemtype="http://schema.org/Event">
      ...
      <div class="event_time">
          <div class="flex flex-col mb-1 mt-1"><h2>Организатор</h2><span itemprop="organizer">{$post->getAttr(9)}</span></div>
          <div class="flex flex-col mb-1 mt-1"><h2>Начало</h2> <span itemprop="startDate" content="{$post->getAttr(7)->format('Y-m-d,H:i')}">{$post->getAttr(7)->format('d.m.Y, H:i')}</span></div>
          <div class="flex flex-col mb-1 mt-1"><h2>Окончание</h2> <span itemprop="endDate" content="{$post->getAttr(8)->format('Y-m-d')}">{$post->getAttr(8)->format('d.m.Y, H:i')}</span></div>
          <div class="flex flex-col mb-1 mt-1" itemprop="location" itemscope itemtype="https://schema.org/Place">
          <div class="flex flex-col mb-1 mt-1"><h2>Локация</h2> <span itemprop="name">{$post->getAttr(11)}</span></div>
          {if ($post->getAttr(10))}
              <div class="flex flex-col mb-1 mt-1"><h2>Адрес</h2> <span itemprop="address">{$post->printAttr(10)}</span></div>
          {/if}
          </div>
          {if ($post->getAttr(6))}
              <div class="flex flex-col mb-1 mt-1"><h2>Сайт</h2> <a href="{$post->getAttr(6)}" target="_blank" itemprop="url" rel="nofollow">{$post->getAttr(6)}</a></div>
          {/if}
      </div>
    </div>
</div>
