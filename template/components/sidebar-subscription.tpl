...
<form action="/!Mail" class="subscription__form sidebar mb-4">
    <a href="{$digest->getUrl()}">
        <img data-src="/img/thumbup.240.320{if $digest->getImage()}{$digest->getImage()->getUrl()}{else}/images/newsletters/{$digest->getPubDate()->format('Y-n')}.png{/if}" data-src="{if $digest->getImage()}{$digest->getImage()->thumbup(120,170)->getUrl()}{else}/img/thumbup.120.170/images/newsletters/{$digest->getPubDate()->format('Y-n')}.png{/if}" alt="Обложка последнего выпуска журнала" class="lazyload subscription__img">
    ...
    <div class="subscription__row">
        <label>
            {if $lang === 'ru'}Номер телефона{else}Phone number{/if}
        </label>
    ...
    <div class="subscription__check-block">
        <div class="subscription__check-item" onclick="changeCheckBox('ru')">
            ...
            <span class="subscription__check-label">
                    {if $lang === 'ru'}Русский{else}Russian{/if}
            </span>
        ...
        <div onclick="changeCheckBox('en')" class="subscription__check-item">
                ...
                <span class="subscription__check-label">
                        {if $lang === 'ru'}Английский{else}English{/if}
                </span>
