...
{if $lang == 'en'}
	...
	{if $post->getAttr(25) != 0}<li><b>Land area:</b> {$post->getAttr(25)} га</li>{/if}
...
{else}
	{if $post->hasAttr(24) != 0}<li><b>Целевое назначение земельного участка:</b> {$post->printAttr(24)}</li>{/if}
	{if $post->getAttr(25) != 0}<li><b>Площадь земельного участка:</b> {$post->getAttr(25)} га</li>{/if}
	{if $post->getAttr(15) != 0}<li><b>Стоимость продажи земельного участка:</b> ${$post->getAttr(15)|number_format:0:'.':' '}</li>{/if}
