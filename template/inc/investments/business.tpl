...
{elseif $lang == 'uk'}
	...
	{/if}
	{if $post->getAttr(15)}<li><b>Ціна:</b> ${$post->getAttr(15)|number_format:0:'.':' '}</li>{/if}
	{if $post->getAttr(17)}<li><b>Продаж частки бізнесу:</b> {$post->getAttr(17)}%</li>{/if}
	{if $post->getAttr(18)}<li><b>Термін існування:</b> {$post->getAttr(18)}</li>{/if}
	{if $post->getAttr(19)}<li><b>Кількість співробітників:</b> {$post->getAttr(19)}</li>{/if}
	{if $post->getAttr(20)}<li><b>Окупність бізнесу, років:</b> {$post->getAttr(20)}</li>{/if}
	{if $post->getAttr(21)}<li><b>Оборот, за останній рік:</b> {$post->getAttr(21)|number_format:0:'.':' '} USD</li>{/if}
	{if $post->getAttr(22)}<li><b>EBITDA, за останній рік:</b> {$post->getAttr(22)|number_format:0:'.':' '} USD</li>{/if}
	{if $post->getAttr(23)}<li><b>Чистий прибуток, USD, за останній рік:</b> {$post->getAttr(23)|number_format:0:'.':' '} USD</li>{/if}
{else}
	{if $post->hasAttr(16)}
    {$t = $post->printAttr(16)->getChilds()}
