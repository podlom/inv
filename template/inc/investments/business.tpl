...
{if $lang == 'en'}
	...
	{if $post->getAttr(18)}<li><b>Life period of business:</b> {$post->getAttr(18)} г.</li>{/if}
	{if $post->getAttr(19)}<li><b>Number of employees:</b> {$post->getAttr(19)} чел.</li>{/if}
	{if $post->getAttr(20)}<li><b>Payback period:</b> {$post->getAttr(20)} г.</li>{/if}
...
{else}
	...
	{/if}
	{if $post->getAttr(15)}<li><b>Стоимость продажи бизнеса:</b> ${$post->getAttr(15)|number_format:0:'.':' '}</li>{/if}
	{if $post->getAttr(17)}<li><b>Предлагаемая к продаже доля бизнеса:</b> {$post->getAttr(17)}%</li>{/if}
	{if $post->getAttr(18)}<li><b>Срок существования бизнеса:</b> {$post->getAttr(18)} г.</li>{/if}
	{if $post->getAttr(19)}<li><b>Количество сотрудников:</b> {$post->getAttr(19)} чел.</li>{/if}
	{if $post->getAttr(20)}<li><b>Окупаемость бизнеса:</b> {$post->getAttr(20)} г.</li>{/if}
