...
{elseif $lang == 'uk'}
	{if $post->hasAttr(24) != 0}<li><b>Цільове призначення:</b> {$post->printAttr(24)}</li>{/if}
	{if $post->getAttr(25) != 0}<li><b>Площа:</b> {$post->getAttr(25)} га</li>{/if}
	{if $post->getAttr(15) != 0}<li><b>Вартість продажу:</b> ${$post->getAttr(15)|number_format:0:'.':' '}</li>{/if}
	{if $post->hasAttr(26)}<li><b>Комунікації:</b> {$post->getAttr(26)}</li>{/if}
	{if $post->getAttr(27)}<li><b>Форма власності:</b> {$post->printAttr(27)}</li>{/if}
{else}
