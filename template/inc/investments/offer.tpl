...
{elseif $lang == 'uk'}
	...
	{/if}
	{if $post->hasAttr(28) != 0}<li><b>Форма інвестування:</b> {$post->printAttr(28)}</li>{/if}
	{if $post->hasAttr(29) != 0}<li><b>Тип інвестора:</b> {$post->printAttr(29)}</li>{/if}
	{if $post->getAttr(30) != 0}<li><b>Мінімальна сума інвестицій:</b> ${$post->getAttr(30)|number_format:0:'.':' '}</li>{/if}
	{if $post->getAttr(31) != 0}<li><b>Максимальна сума інвестицій:</b> ${$post->getAttr(31)|number_format:0:'.':' '}</li>{/if}
{else}
	{if $post->hasAttr(16)}
    {$t = $post->printAttr(16)->getChilds()}
