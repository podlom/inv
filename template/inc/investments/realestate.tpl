{if $lang == 'en'}
	...
	{if $post->getAttr(41) != 0}<li><b>Total area:</b> {$post->getAttr(41)} кв.м.</li>{/if}
	{if $post->getAttr(15) != 0}<li><b>Price:</b> ${$post->getAttr(15)|number_format:0:'.':' '}</li>{/if}
	{if $post->getAttr(42) != 0}<li><b>Total land area:</b> {$post->getAttr(42)} га</li>{/if}
...
{else}
	...
	{if $post->getAttr(15) != 0}<li><b>Стоимость продажи недвижимости:</b> ${$post->getAttr(15)|number_format:0:'.':' '}</li>{/if}
