...
var LIMITED_CHECKBOX = 'input[name="sf_investor[types][]"], input[name="sf_investor[profit][]"]';

if ($(LIMITED_CHECKBOX).length) {
  $(LIMITED_CHECKBOX).click(function () {
    var selector = "input[name=\"".concat($(this).attr('name'), "\"]");
...
function changeCheckBox(lang) {
  if (lang == 'ru') {
    document.getElementById('check-ru').checked = !document.getElementById('check-ru').checked;
    document.getElementById('check-box-ru').classList.toggle('subscription__check-box--active');
  }

  if (lang == 'en') {
    document.getElementById('check-en').checked = !document.getElementById('check-en').checked;
    document.getElementById('check-box-en').classList.toggle('subscription__check-box--active');
  }
}
...
$('form.subscribe').submit(function (e) {
  e.preventDefault(); // @sr 2020-01-22 do not send form with wrong phone inputmask value
...
$('form[action="/form/approach"], form[action="/form/investor"], form[action="/form/investor_en"], form[action="/form/investment_callback"]').submit(function (e) {
  e.preventDefault(); // @ts 2020-01-17 do not send form with wrong phone inputmask value
...
$(document.body).ready(function () {
  if ($('.section__category').length) {
    ...
    var activeParent = url.searchParams.get('filter[category][parent]');
    var sortType = url.searchParams.get('filter[sort]');
    ...
    $('.section__category').each(function (el) {
      ...
      if (sortType) {
        ...
        $(this).attr('href', "".concat(_link, "&filter[sort]=").concat(sortType));
      }
    });
  }

  if ($('.preview__name-category').length) {
    var _link2 = $('.preview__name-category a').attr('href');

    _link2 = _link2.split('/').filter(function (i) {
      return !!i;
    });

    if (Array.isArray(_link2) && _link2.length && (_link2[0] === 'investments' || _link2[1] === 'investments')) {
      $('.preview__name-category a').attr('href', "/investments?filter[category][parent]=".concat(_link2[1]));
    }
  }

  if (window.location.pathname.indexOf('/investments') !== -1) {
    console.log('fetch invest');
    var search = window.location.search;
    var query = search ? "".concat(search, "&action=get") : "?action=get";

    var _link3 = "/invest.php".concat(query);

    $.get(_link3).then(function (data) {
      if (data) {
  ...
  $('#gm_callback form').submit(function (e) {
    ...
    if ($('#phone3').length && $('#error-msg3').length && !$('#error-msg3').hasClass('hide')) {
...
$(document).ready(function () {
  ...
  if ($('[data-tippy-content]').length) {
    Promise.all([loadScript('https://unpkg.com/@popperjs/core@2.4.4/dist/umd/popper.min.js'), loadScript('https://unpkg.com/tippy.js@6')]).then(function () {
  ...
  if ($('#events').length || $('.event_list').length) {
    ...
    var action = pathname.includes('tools/events') ? 'events' : 'home';
    $.get("/event.php?action=".concat(action)).then(function (data) {
      if (data) {
  ...
  loadScript('https://cdnjs.cloudflare.com/ajax/libs/remodal/1.1.1/remodal.min.js');
  var selectConfig = {
    selectAll: false,
    formatCountSelected: function formatCountSelected(count, total) {
      var fromStr = document.documentElement.lang === 'ru' ? ' из ' : ' of ';
      return count + fromStr + total;
    },
    formatAllSelected: function formatAllSelected() {
      return document.documentElement.lang === 'ru' ? 'Все выбраны' : 'All selected';
    }
  };
  ...
  if ($('#branch').length || $('#filter__type').length || $('#regions').length) {
    ...
    var paramsArray = groupParamsByKey(params);

    var _loop = function _loop(key) {
      var val = paramsArray[key];

      if (val) {
        $("[name=\"".concat(key, "\"] option")).each(function (_, el) {
          var optionSelected = Array.isArray(val) && val.includes(el.value) || val == el.value;
    ...
    if ($('.content table').length) {
      loadScript('https://cdn.datatables.net/1.10.21/js/jquery.dataTables.js').then(function () {
        $('.content table').addClasss('stripe');
        $('.content table').DataTable();
      });
    }

    loadScript('/js/multiple-select.min.js').then(function () {
      if ($('#branch').length) {
  ...
  $('#invest-sort').change(function () {
    var sortType = $('#invest-sort').val();
    window.location.href = sortType;
  });
  $('.toggle-filter').click(function () {
    $('.filter__wrapper').toggleClass('opened');
  ...
  function openMenu() {
    ...
    $('#open-mobile-menu').addClass('header__btn--close').removeClass('header__btn--burger');
  }

  function closeMenu() {
    $('#open-mobile-menu').removeClass('header__btn--close').addClass('header__btn--burger');
    $('body, html').removeClass('modal-open');
    $('#mobile-nav').removeClass('mobile-nav--opened');
  }

  $('#open-mobile-menu').click(function () {
    $('#open-mobile-menu').hasClass('header__btn--close') ? closeMenu() : openMenu();
  });

  if ($('#search-page #sort-select').length) {
    var activeClass = 'sort__item--active';
    ...
    var sortType = $('input#sort').attr('value');
    if (!sortType) sortType = 'rel';
    $("#sort-select li[data-value=\"".concat(sortType, "\"]")).addClass(activeClass);
  }

  if ($('.post__video').length) {
    var postId = $('.post__video').attr('data-id');
    var elementToHide = document.getElementById('hide-if-text-empty');

    if (elementToHide) {
      $.get("/page-attr-data.php?page_id=".concat(postId, "&action=get_text")).then(function (data) {
